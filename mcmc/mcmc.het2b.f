...
c --- compute variances
	...
	DEL(1:NPAR)=.001d0  !perhaps use dstep
 	...
 	CALL HESSIAN(NPAR,NDIM,IVN,X0,Y,DF,DDF,W,NCT)
...
c ---  generate lambda_i's, if L>0 / initially all lambda's equal or close
	...
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        WRITE(9,1001) (SR(I)/SA(I),I=1,LH), (SQRT(SR(I))/SA(I),I=1,LH)
        ELSE
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        ENDIF
...
c --- step 0 (theta's) / always carried out
...
c	ENDDO
        IF(IMCMC.EQ.1) SU0(IRE(1:LH))= SR(1:LH)/SA(1:LH)  
