...
c     imcmc=2: do heterogeneity problem via mcmc
      ...
      REAL*8, DIMENSION(4000) :: LKH,LKH0
      REAL*8, DIMENSION(1,4000) :: LAMBDA,LAMBDA0,TLAM,TLAM0
...
c ---  initialization of sa and sb / here only for 1 parameter / generalize later
        ...
        IF(IMCMC.EQ.1) THEN
c --- on logit scale
        XMU(1:LH)=X0(IRE(1:LH))   ! start with mean (mu) = mle
 74     PRINT*,'initial sd parameter (on log/logit scale):'
        READ*,sig0
        SIG(1:LH)=sig0              ! start with SE(IRE(1:LH)) ?

c ---  generate lambda_i's, if L>0 
        DO I=1,LH
           DO K=1,NOBS
           TLAM(I,K)=GENNOR(0.,SIG(I))
           YVAL=XMU(I)+TLAM(I,K) !gengam(real(sa),real(sr))
           RHO=DEXP(YVAL)
           LAMBDA(I,K)=(RHO*S2(IRE(I))+S1(IRE(I)))/(1.D0+RHO)
           ENDDO
        ENDDO
	...
	CALL FUNC(SU0,NDIM,F0); NCT=NCT+1 !fills lkh's
        ...
        PRINT*,'continue (1) or select new sd parameter (0):'
        READ*,i01
        IF(i01.eq.0) goto 74
	...
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        WRITE(9,1001) (XMU(I),I=1,LH), (SIG(I),I=1,LH)
        ELSE
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        ENDIF
...
c --- compute mean+random deviation (mu+lambda) using proposals on means
        ...
        CALL FTRAFO(NDIM,XP,Y)

        DO I=1,LH    !on logit scale
           DO K=1,NOBS
           YVAL=XP(IRE(I))+TLAM(I,K)
           RHO=DEXP(YVAL)
           LAMBDA(I,K)=(RHO*S2(IRE(I))+S1(IRE(I)))/(1.D0+RHO)
           ENDDO
        ENDDO
...
c --- write
	...
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        WRITE(9,1001) (XP(IRE(I)),I=1,LH), (SIG(I),I=1,LH)
        ELSE
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        ENDIF
        close(8); close(9)
