...
c     note: uses gamma hetero distributions

c     note alngam requires real*8 arguments
c     note lkh... usually refers to 'negative log-likelihood'

c     W: Hessian eigen values
c     Z: Hessian eigen vectors

      SUBROUTINE MCMC_HET_GAM(NDIM,NPAR,LH,NCT,X0,F0,MEANV,COVM,X)
      ...
      DIMENSION SQ(NMAX),SU0(NMAX),SU00(NMAX),Z(NMAX,NMAX)
...
! 55   CLOSE(7)
      ...
      N_ACC=0; N_ACC_LAMBDA=0; N_ACC_SR=0

        OPEN(8,FILE='mcmc.out',STATUS='unknown')
        IF(IMCMC.EQ.1) THEN
	WRITE(8,2001) LABELS(IVN(1:NPAR)),LABELS(IRE(1:LH)) 
! 2001   FORMAT('ID ','nllkh ',<npar>a10,<lh>a10)
 2001   FORMAT('ID ','nllkh ',100a10)
        ELSE
	WRITE(8,2002)  LABELS(IVN(1:NPAR))
! 2002   FORMAT('ID ','nllkh ',<npar>a10)
 2002   FORMAT('ID ','nllkh ',100a10)
        ENDIF
        close(8)

      IF(COVM(1,1) .NE. 0.) THEN
         PRINT*,'have COVM, will directly procede to pilot run 2'
         PRINT 999
         SM=0.
         GOTO 75
      ENDIF
...
c ---------------------------------------------
        ...
        IF(MOD(N,100).EQ.0) THEN
         WRITE(6,1007) N,N_ACC,N_ACC_SR,(N_ACC_LAMBDA(I),I=1,LH)
...
c ---  generate lambda_i's, if L>0 / initially all lambda's equal or close
	...
	CALL FUNC(SU0,NDIM,F0); NCT=NCT+1
        ...
        IF(IMCMC.EQ.1) THEN
	WRITE(8,1001) mc_id,F0,(SU0(IVN(I)),I=1,NPAR),(SU0(IRE(I))/DSQRT(SR(I)),I=1,LH)
        ELSE
...
c     which are Gibbs sampled.
	...
	Y(IVN(I))=SU0(IVN(I))+scl*X(I)
	  ...
	  IF(Y(IVN(I)).LT.S1(IVN(I))) THEN
           PRINT*,'down move rejected!  ',LABELS(IVN(i))
           GOTO 14
          ELSEIF(Y(IVN(I)).GT.S2(IVN(I))) THEN
           PRINT*,'up move rejected!    ',LABELS(IVN(i))
           GOTO 14
          ENDIF
        ENDIF
...
c ---  step1: generate eta (b/a) | sr (a),lambda using Gibbs

           TMP=0. !for importance sampling

           DO I=1,LH  !for each heterogeneous parameter
            a=sr(i)
            sum_lambda=sum(lambda(i,1:nobs))
            a_sum_lambda=a*sum_lambda
            eta=gengam(real(a_sum_lambda),real(nobs*a+1.)) 
            su0(ire(i))=1./eta

c ---  step2: generate sr (a) | b/a (mean),lambda using MH

            G0=-nobs*alngam(a)+a*sum(dlog(lambda(i,1:nobs)*a*eta))
            G0=G0-a*eta*sum_lambda

            a_=genunf(real(.8*a),real(1.2*a))

            GP=-nobs*alngam(a_)+a_*sum(dlog(lambda(i,1:nobs)*a_*eta))
            GP=GP-a_*eta*sum_lambda

c           print*,a,a_

c --- if chosen a is less than 1.d-6 (lower bound) reject       
            if(a_.le.1.d-6) goto 12
            
            ACC_G=MIN(1.D0,a*DEXP(GP-G0)/a_)
 12         IF(ACC_G.EQ.1.D0) THEN
               sr(i)=a_; N_ACC_SR=N_ACC_SR+1
            ELSE
         ...
         if(mod(n,100).eq.0) then; print*,i,su0(ire(i)),su0(ire(i))/dsqrt(sr(i)); endif

c ---  step 3: generate new lambda_i's | a,eta

        ACC_P=0.; IERR=0

        LAMBDA0=LAMBDA; LKH0=LKH

        DO K=1,NOBS
!         lkh0(i)   =lkh(i) !we have this already form sampling Y
         lambda(i,k)=gengam(real(sr(i)*eta),real(sr(i)))
        ENDDO
...
c --- use Metropolis-Hastings here to accept or reject the lambda_i's
        ...
        DO K=1,NOBS
         ...
         IF(ACC_P.EQ.1.D0) THEN
          LAMBDA0(I,K)=LAMBDA(I,K); LKH0(K)=LKH(K)
...
c ---------------------------------------------
	...
	N_ACC=0; N_ACC_LAMBDA=0; N_ACC_SR=0 !resetting the acceptance counter to zero
...
c ---  final analysis - return approx. mle
 ...
 1007  FORMAT('cycle:',I6,' MVN accept:',I4,' (shape) accept:',I4,' (lambda) accept:',30I4)
