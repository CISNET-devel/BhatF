...
C ---------------------------------------------------------------------- IF HELP
	...
	PRINT*,'boot   :	Run bootstrap (assumes that sampling is implemented)'
        PRINT*,'ci     :        Compute profile likelihood CIs of par <lbl>'
	PRINT*,' '
 ...
 22        WRITE(6, FMT='(A7)', ADVANCE='NO') 'Label: '
           ...
           DO I=1,NDIM
              IF(FLBL.EQ.LABELS(I)) THEN
                 ...
                 IF(RNF(I).EQ.'S') PRINT*,'note, this is a STOPPED parameter'
                 IF(RNF(I).EQ.'R') PRINT*,'note, this is a RUNNING parameter'
                 WRITE(6, FMT='(A12)', ADVANCE='NO') 'what value: '
                 READ(5, FMT='(F10.0)', ADVANCE='YES', ERR=25) SU(I)
                 IF(SU(I).GE.S2(I) .OR. SU(I).LE.S1(I)) THEN
                    PRINT*,'value is out of selected bounds (use reset to fix)'
                    GOTO 23
                 ENDIF
                 CALL FTRAFO(NDIM,X0,SU)  
                 XS0 = X0
...
8003    FORMAT('DERIV. LIMIT (0.01), ITERATIONS (10), RETRACTION(0/1): ')
...
!!      INCLUDE '/usr/pgi/linux86/include/lib3f.h'
      CALL SYSTEM('if [ -f mcmc.out ]; then mv -f mcmc.out mcmc.out.old; fi')
      CALL SYSTEM('if [ -f mcmc.het.out ]; then mv -f mcmc.out mcmc.het.out.old; fi')
