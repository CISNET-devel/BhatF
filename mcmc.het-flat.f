...
c     note alngam requires real*8 arguments
      ...
      REAL*8, DIMENSION(NMAX) :: SR,SR_,SA,SA_
...
C --- start 1. training M chain (on logit scale) -------------------
...
c -------------------------------------------------------------------------
c ---  check for heterog. parameters (using RNF (run flag) in bhat.inp)
...
c ---  initialization of a (sa) and b (sr) 
...
c        READ*,SR00
           ...
           SR(1:LH)=SE(IRE(1:LH)) ! width b
           SA(1:LH)=SU0(IRE(1:LH)) !mean a

c ---  generate lambda_i's, if L>0 / initially all lambda's equal or close
        DO I=1,LH
         LAMBDA(I,1:NOBS)=SU0(IRE(I))
        ENDDO
	...
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        WRITE(9,1001) (SA(I),I=1,LH), (SR(I),I=1,LH)
        ELSE
	WRITE(8,1001) F0,(SU0(IVN(I)),I=1,NPAR)
        ENDIF
...
c --- note that if any nuissance parameter is out of bounds then rejection
...
c ---  step1: generate width b (sr) | mean a (sa), lambda_i's using MH sampler
...
c --- if chosen b is less than 1.d-6 (lower bound) reject       
            if(sr_(i).le.1.d-6) goto 12

c --- if |a-lambda_i|>b/2 reject

        DO K=1,NOBS
            if(dabs(sa(i)-lambda(i,k)) > .5*sr_(i)) goto 12
        ENDDO
            
            ACC_G=MIN(1.D0,(sr(i)/sr_(i))**(nobs+1))
...
c ---  step2: generate mean a (sa) | width b (sr),lambda_i's using MVN

        DO I=1,LH
         ACC_G=0.

c --- try runf proposal with width dep. on current point
         sa_(i)=genunf(real(.975*sa(i)),real(1.025*sa(i)))  !y(ire(i)) 

c --- if chosen a is less than 1.d-6 (lower bound) reject       
            if(sa_(i).le.1.d-6) goto 11

c --- if |a-lambda_i|>b/2 reject
         DO K=1,NOBS
            if(dabs(sa_(i)-lambda(i,k)) > .5*sr(i)) goto 11
         ENDDO

            ACC_G=MIN(1.D0,(sa(i)/sa_(i)))

 11         IF(ACC_G.EQ.1.D0) THEN
               sa(i)=sa_(i); n_acc_sa=n_acc_sa+1
            ELSE
               IF(genunf(0.,1.).LE.ACC_G) THEN
                  sa(i)=sa_(i); n_acc_sa=n_acc_sa+1
               ENDIF
            ...
            if(mod(n,100).eq.0) then; print*,i,sa(i),sr(i); endif
       ENDDO

c ---  step 3: generate new lambda_i's | sa(a),sr(b)
...
!         lkh0(i)   =lkh(i) !we have this already form sampling Y
         lambda(i,k)=genunf(real(sa(i)-.5*sr(i)),real(sa(i)+.5*sr(i)))
         if(lambda(i,k).gt.s2(ire(i)).or.lambda(i,k).lt.s1(ire(i))) then
            print*,'lambda rejected ',i,k,lambda(i,k)
            lambda(i,k)=lambda0(i,k) !reject and keep current value
            ireject(k)=1
         endif
        ENDDO
        ENDDO
...
c	ENDDO
        IF(IMCMC.EQ.1) SU0(IRE(1:LH))= SA(1:LH)  

        WRITE(6,1007) N,N_ACC,N_ACC_LAMBDA,N_ACC_SA,N_ACC_SR,F0,(SU0(IVN(I)),I=1,NPAR)
