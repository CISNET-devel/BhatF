...
c     note alngam requires real*8 arguments
      ...
      SUBROUTINE MCMC_HET_GAM(NDIM,NPAR,NCT,X0,F0,MEANV,COVM,X)
...
! -----------------------------------------------------------------
        ...
        IF(IMCMC.EQ.1) THEN
        IF(MC_S.GE.0) THEN !now always! otherwise use MH sampler for a in first pilot run
...
!!            sr_(i)=genunf(real(.9*sr(i)),real(1.1*sr(i)))
            sr_(i)=genunf(real(sr(i)-1.),real(sr(i)+1.))
...
c	ENDDO
        XM=SU0
        IF(IMCMC.EQ.1) XM(IRE(1:LH))= SR(1:LH)/SA(1:LH)
...
c        IDUM=SYSTEM('killall gnuplot')
c        IDUM=SYSTEM('killall gnuplot_x11')
