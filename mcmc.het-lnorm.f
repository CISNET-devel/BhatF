...
c ---  generate lambda_i's from log-normal distribution
	...
	CALL FUNC(SU0,NDIM,F0); NCT=NCT+1
        ...
        IF(IMCMC.EQ.1) THEN
	WRITE(8,1001) mc_id,F0,(SU0(IVN(I)),I=1,NPAR),(SIG(I),I=1,LH)
        ELSE
...
c --- compute mean+random deviation (mu+lambda) using proposals on means
        ...
        DO I=1,LH    !on logit scale
           YMP=DLOG(Y(IRE(I)))   !log median, proposed
           YMC=DLOG(SU0(IRE(I))) !log median, current
           RHOI=0.
           DO K=1,NOBS
              YLAMIK=DLOG(LAMBDA(I,K))
              RHOI=RHOI+(YLAMIK-YMC)**2-(YLAMIK-YMP)**2
           ENDDO
           RHOI=RHOI/2./SIG(I)/SIG(I)
           RHO=RHO+RHOI
        ENDDO
...
C ---	R ratio ---------------------------------------------------
        ...
        ACC_P=MIN(1.D0,DEXP((-Fnew+Fold+RHO)))
...
c ---------------------------------------------
        ...
        DO I=1,LH
           ...
           PRINT*,SIG(I),SU0(IRE(I))*DSQRT(WSIG*WSIG-WSIG)
        ENDDO
        WRITE(6,1007) N,N_ACC,(N_ACC_LAMBDA(I),I=1,LH)
        WRITE(6,1006) F0,(SU0(IVN(I)),I=1,NPAR)
